...
--?ûê?ú†Î°?Í≤? in, out?ùÑ ?çî Ï∂îÍ??ïò?Öî?èÑ ?ê©?ãà?ã§.
    Port (
			m_reset 		: in STD_LOGIC;
			m_clk 			: in STD_LOGIC;
			m_en			: in STD_LOGIC;
...
signal s_m_8254_wr_b              : std_logic;
...
signal s_ctrl_reset : STD_LOGIC;
signal s_optmode_m_reset : STD_LOGIC;
signal s_optmode_m_valid : STD_LOGIC;
...
clk_gen : TOP_8254 port map(
           ...
           m_clk2    => s_clk,
           m_clk_ctr => s_clk,
           m_reset   => s_m_8254_reset, --8254 resetÍ≥? ?òº?ö©?ê† ?àò ?ûà?èÑÎ°? Î≥?Í≤?
...
s_ctrl_reset <= NOT m_fpga_reset OR s_reset_addr;
...
OPTMODE_CTRL : entity work.Option_mode (Behavioral)
   port map (
			m_reset => s_optmode_m_reset,
			m_clk 	=> s_clk,

			m_xin 		=> s_PC_RAM_dout,
			m_xin_valid => s_optmode_m_valid,
...
s_optmode_m_reset <= s_OPTMODE_CTRL_rst OR s_reset_addr OR NOT m_fpga_reset;
s_optmode_m_valid <= s_PCRAM_CTRL_r_rdy AND s_OPTMODE_CTRL_en;
...
-- s_debug_led, s_debug_sw, s_debug_headerÎ•? debugging?óê ?†ÅÍ∑? ?ôú?ö©?ïò?Ñ∏?öî.
